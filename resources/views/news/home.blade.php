...
@section('content')
            ...
            <div class="flex flex-col gap-4">
                @foreach ($rest as $item)
                    <a href="{{ route('article.show', $item) }}" class="group flex gap-3">
                        ...
                        <div>
                            @if ($item->category)
                                <span class="text-xs font-bold uppercase text-[var(--brand)]">{{ $item->category->name }}</span>
                            @endif
                            <h3 class="text-sm font-bold leading-snug group-hover:text-[var(--brand)]">{{ $item->title }}</h3>
                        </div>
                    </a>
                @endforeach
            </div>
    ...
    <div class="mt-10 grid gap-10 lg:grid-cols-3">
        ...
        <div class="lg:col-span-2">
            <h2 class="mb-4 border-b-2 border-[var(--brand)] pb-2 text-lg font-black uppercase">Aktuelle Nachrichten</h2>
            ...
            @foreach ($categoryBlocks as $cat)
                <section class="mt-10">
                    <div class="mb-4 flex items-center justify-between border-b-2 border-gray-900 pb-2">
                        ...
                        <a href="{{ route('category.show', $cat) }}" class="text-sm font-semibold text-[var(--brand)] hover:underline">Alle ansehen →</a>
                    </div>
                    <div class="grid gap-6 sm:grid-cols-2">
        ...
        {{-- Sidebar --}}
        <aside>
            <h2 class="mb-4 border-b-2 border-[var(--brand)] pb-2 text-lg font-black uppercase">Meistgelesen</h2>
            <ol class="space-y-4">
                @foreach ($mostViewed as $i => $article)
                    <li class="flex gap-3">
                        <span class="text-2xl font-black text-gray-300">{{ str_pad($i + 1, 2, '0', STR_PAD_LEFT) }}</span>
                        <a href="{{ route('article.show', $article) }}" class="text-sm font-semibold leading-snug hover:text-[var(--brand)]">
                            {{ $article->title }}
